...
spec = FlaskPydanticSpec('flask', title='API Fanfic de Rick and Morty')
spec.register(app)
...
def inserir_personagem():
    ...
    insertLocation = body.get('location').lower().strip()
    body.update({'location': insertLocation})
    local = ""
    try:
    ...
    idLocal = len(db.all()) + 1
    insertOrigin = body.get('origin').lower().strip()
    body.update({'origin': insertOrigin})
    origin =""
    try:
...
def inserir_Local():
    ...
    body.update({'nome': body.get('nome').lower().strip()})  
    db.insert(body)
    return body
